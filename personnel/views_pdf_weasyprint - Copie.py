...
def liste_responsables_coordonnateurs_pdf(request):
    ...
    for i, agent in enumerate(agents, start=1):
        donnees.append([
            ...
            calcul_duree(agent.date_affectation)
        ])
    ...
    for service, groupes in sorted(groupes_services.items()):  # tri par nom de service
        ...
        def fonction_cadre_index(emp):
            priorites = [
                ...
                ("Directeur", 1),
                ("Assistant Administratif/DG", 2),
                ("Assistant Financier/DG", 3),
                ("Assistant Chargé de Recouvr./DG", 4),
                ("Assistant Chargé de Mission/DG", 5),
                ("Assistant Juridique/DG", 6),
                ("Assistant Technique/DG", 7),
                ("Assistant du DGA", 8),
                ("Sous-Directeur GESOC", 9),
                ("Sous-Directeur Pension Compl.", 10),
                ("Sous-Directeur de Trésorerie", 11),
                ("Sous-Directeur Log. et Maint.", 12)
                ("Sous-Directeur des Appro", 13)
                ("Sous-Directeur Juridique", 14)
                ("Sous-Directeur Contentieux", 15)
                ("Sous-Directeur de Grandes Entreprises", 16)
                ("Sous-Directeur des Statistiques", 17)
                ("Sous-Directeur du Contentieux", 18)
                ("Sous-Directeur Pharmacie et Labo", 19)
                ("Sous-Directeur Technique", 20),
                ("Sous-Directeur Adm. et Fin.", 21),
                ("Sous-Directeur", 22),
        ]
...
def liste_retraites_pdf(request):
    agents = Employe.objects.filter(statut__iexact='Mise à la retraite').order_by('nom')
    donnees = []
    ...
    for idx, emp in enumerate(agents, 1):
        ...
        if emp.date_naissance and emp.date_statut:
            age = calcul_duree(emp.date_naissance)
            age_retraite = format_duree(age)

        if emp.date_engagement and emp.date_statut:
            duree = calcul_duree(emp.date_engagement)
            duree_carriere = format_duree(duree)
    ...
    context = {
        ...
        "colonnes": ["N°", "Nom", "Matricule", "Sexe", "Date de naissance", "Date départ retraite", "Âge départ", "Date engagement", "Carrière", "Entité"],
        "donnees": donnees
    }

    html_string = render_to_string("personnel/liste_retraites_pdf.html", context)
    response = HttpResponse(content_type="application/pdf")
...
def liste_decedes_pdf(request):
    ...
    for idx, emp in enumerate(agents, 1):
        age = "-"
        if emp.date_naissance and emp.date_statut:
            age_val = calcul_duree(emp.date_naissance)
            age = format_duree(age_val)
        donnees.append([
            idx,
            emp.nom or '-',
            emp.matricule or '-',
    ...
    html_string = render_to_string("personnel/liste_decedes_pdf.html", {
        ...
        "colonnes": ["N°", "Nom", "Matricule", "Sexe", "Date de naissance", "Date décès", "Âge au décès", "Entité"],
        "donnees": donnees
    })
    response = HttpResponse(content_type="application/pdf")
